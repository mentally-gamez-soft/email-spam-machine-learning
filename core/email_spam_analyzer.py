...
class EmailSpamAnalyzer:
    ...
    def display_ratio_non_spam_email(self):
        print('The ratio for non spam email is: ', round(len(self.df[self.df['Label']==0])/len(self.df['Label']),2)*100,'%')

    def display_ratio_spam_email(self):
        print('The ratio for spam email is: ', round(len(self.df[self.df['Label']==1])/len(self.df['Label']),2)*100,'%')
    ...
    def graph_display_message_distribution_before_data_sanitizing(self):
        ...
        sns.distplot(self.df[self.df['Label']==1]['length'],bins=20,ax=ax[0],label='Spam Message Distribution before data sanitizing',color='r')
        ax[0].set_xlabel('spam message length before data sanitizing')
        ax[0].legend()

        sns.distplot(self.df[self.df['Label']==0]['length'],bins=20,ax=ax[1],label='No Spam Message Distribution before data sanitizing',color='b')
    ...
    def graph_display_message_distribution_after_data_sanitizing(self):
        f, ax = plt.subplots(1,2,figsize=(15,8))
        sns.distplot(self.df[self.df['Label']==1]['clean_length'],bins=20,ax=ax[0],label='Spam Message Distribution after data sanitizing',color='y')
        ax[0].set_xlabel('spam message length after data sanitizing')
        ax[0].legend()

        sns.distplot(self.df[self.df['Label']==0]['clean_length'],bins=20,ax=ax[1],label='No Spam Message Distribution after data sanitizing',color='g')
    ...
    def display_wordcloud_for_spam_emails(self):
        ...
        spam_cloud = WordCloud(width=1024,height=768,background_color='white',max_words=500).generate(''.join(spams))
    ...
    def display_wordcloud_for_non_spam_emails(self):
        ...
        no_spam_cloud = WordCloud(width=1024,height=768,background_color='white',max_words=500).generate(''.join(no_spams))
        plt.figure(figsize=(11,9),facecolor='b')
    ...
    def __remove_stop_words(self):
        ...
        self.df['EmailText'] = self.df['EmailText'].apply(lambda x:" ".join(term for term in x.split() if term not in stopwords))

    def sanitize_data(self):
        self.df['EmailText'] =  self.data_sanitizer.sanitize_web_url(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_email(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_currency(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_leading_whitespace(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_number(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_phonenumber(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_punctuation(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_trailing_whitespace(overwright_text=True)        
        self.df['EmailText'] =  self.data_sanitizer.sanitize_whitespace(overwright_text=True)
        self.df['EmailText'] =  self.data_sanitizer.sanitize_font(overwright_text=True)
    ...
    def train_model(self):
        ...
        x_train, x_test, y_train, self.y_test = train_test_split(x,y,random_state=42)
